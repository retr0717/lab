  ...
  while (completed < n) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (processes[i].remaining > 0) {
        // Execute process for quantum or remaining burst time, whichever is
        // smaller
        int executionTime = (processes[i].remaining < quantum)
        ...
        // If process is completed, calculate turnaround time and increment
        // completed count
        if (processes[i].remaining == 0) {
...
int main() {
  ...
  // Perform Round Robin scheduling and print the final result along with
